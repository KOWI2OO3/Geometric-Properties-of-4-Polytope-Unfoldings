...
def run_pipeline(min, max, step: float, func):
        ...
        with open("output/results.txt", "w") as results:
            ...
            while True:
                ...
                for m in range(m_instances):
                    ...
                    hull = func(i)
                    success_rate = pipeline_sample(hull, logger)
...
def pipeline_sample(hull, logger):
...
# Experiment 1
# run_pipeline(5, 30, 1, lambda x: gen.generate_clustering_unit_convex(x, 0))

# Experiment 2 ~ 16 hours
# run_pipeline(0, 1, 0.05, lambda x: gen.generate_clustering_unit_convex(12, x))

# Experiment 3 ~ 16 hours
run_pipeline(1, 20, 1, lambda x: gen.generate_convex_with_deformation(10, np.array([x, 1, 1, 1])))